...
def main(args):
    ...
    ckpt_path = args.ckpt if args.ckpt else (args.resume_ckpt_low_resolution if args.resume_ckpt_low_resolution else f"DiT-XL-2-{args.image_size}x{args.image_size}.pt")
    state_dict = find_model(ckpt_path)
    if (args.resume_ckpt_low_resolution is not None) and (os.path.exists(args.resume_ckpt_low_resolution)):
        state_dict= interpolate_state_dict(state_dict, current_state_dict=model.state_dict())
    ...
    save_image(samples, "samples/sample-ttt.png", nrow=4, normalize=True, value_range=(-1, 1))
